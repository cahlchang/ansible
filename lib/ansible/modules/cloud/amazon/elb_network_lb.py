...
description:
    - Manage a AWS Network Elastic Load Balancer. See U(http://docs.aws.amazon.com/elasticloadbalancing/latest/network/introduction.html) for details.
version_added: "2.5"
  ...
  subnets:
    description:
      - A list of the IDs of the subnets to attach to the load balancer. You can specify only one subnet per Availability Zone. You must specify subnets from
        at least two Availability Zones. You must specify either subnets or subnet mappings. Required if state=present.
    required: false
  subnet_mappings:
    description:
      - A list of dicts specifying the IDs of the subnets to attach to the load balancer and also the EIP allocation ID if applicable. You can specify only
        one subnet per Availability Zone. You must specify either subnets or subnet mappings. Required if state=present.
    description:
      - A list of the IDs of the subnets to attach to the load balancer. You can specify only one subnet per Availability Zone. You must specify subnets from
    ...
    if listeners is not None:
        for listener in listeners:
            for key in listener.keys():
                if key not in ['Protocol', 'Port', 'DefaultActions', 'Rules']:
